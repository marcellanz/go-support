...
This can be done manually through the https://github.com/protocolbuffers/protobuf#user-content-protocol-compiler-installation[Protocol Buffer Compiler project].
...
To build the example shopping cart application shown earlier in xref:concepts:grpc.adoc[gRPC descriptors], you could simply paste that protobuf into `protos/shoppingcart.proto`.
...
Cloudstate entities in Go work by implementing interfaces.

To get support for the Cloudstate event emission the Cloudstate entity should embed the `cloudstate.EventEmitter` type.
The EventEmitter allows the entity to emit events during the handling of commands.
...
This entity factory function returns a `cloudstate.Entity` which itself is a composite interface of a `cloudstate.CommandHandler` and a `cloudstate.EventHandler`.
Every event sourced entity has to implement these two interfaces.
