...
Each CRDT entity manages one root CRDT. That CRDT will either be supplied to the entity by the proxy when it is started through {cloudstate-go-lib-api-base}/cloudstate/crdt#EntityHandler.Set[`crdt.EntityHandler.Set`].
If no CRDT exists for the entity when it is started, it has to be created by the entity using the {cloudstate-go-lib-api-base}/cloudstate/crdt#EntityHandler.Default[`crdt.EntityHandler.Default`] factory method.
...
So to handle the shopping cart service method `rpc GetCart (GeShoppingCart) returns (Cart);` the commands name `GetCart` together with its message type `GetShoppingCart` has to be matched.
...
While the Go runtime function `runtime.FuncForPC` allows to get a function pointers pointed function name and there are ways to get the `shoppingcart.ShoppingCartServiceServer.AddItem` command name from the gRPC interface of the shopping cart service, we do not encourage doing so.
...
For the rest of the protocol buffers specification, while no guarantees are made on the stability by the specification itself, the Go protocol buffer libraries do https://github.com/protocolbuffers/protobuf-go/blob/v1.25.0/proto/encode.go#L31-L52[produce stable orderings of fields and stable output of non-map values].
